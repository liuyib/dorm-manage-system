...
namespace Wpfz.Docking.Controls
{
    public class LayoutAnchorableItem : LayoutItem
    {
        ...
        {
            _defaultHideCommand = new Core.RelayCommand<ICommand>((p) => ExecuteHideCommand(p), (p) => CanExecuteHideCommand(p));
            _defaultAutoHideCommand = new Core.RelayCommand<ICommand>((p) => ExecuteAutoHideCommand(p), (p) => CanExecuteAutoHideCommand(p));
            _defaultDockCommand = new Core.RelayCommand<ICommand>((p) => ExecuteDockCommand(p), (p) => CanExecuteDockCommand(p));
